import
  std/json,
  bodies/[broadcast, global, replay_runtime, replays, sim]
...
## used to die with bodies_error_len() == 0 and no diagnostic at all). The
...
proc bodiesLoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "bodies_load_replay", cdecl.} =
  try:
    lastError = ""
...
proc bodiesInput(data: ptr uint8, length: cint)
    {.exportc: "bodies_input", cdecl.} =
  if runtimeLoaded:
    viewer.applyGlobalViewerMessage(data.bytesFromPointer(int(length)))

proc bodiesFrame(): cint {.exportc: "bodies_frame", cdecl.} =
...
proc bodiesPacketPointer(): ptr uint8
    {.exportc: "bodies_packet_ptr", cdecl.} =
  if packet.len == 0:
    nil
  else:
    packet[0].addr

proc bodiesPacketLength(): cint {.exportc: "bodies_packet_len", cdecl.} =
  cint(packet.len)

proc bodiesMismatchTick(): cint {.exportc: "bodies_mismatch_tick", cdecl.} =
  if runtimeLoaded:
    cint(replay.hashMismatchTick)
  else:
    -1

proc bodiesErrorPointer(): ptr uint8 {.exportc: "bodies_error_ptr", cdecl.} =
  if lastError.len == 0:
    nil
  else:
    cast[ptr uint8](lastError[0].addr)

proc bodiesErrorLength(): cint {.exportc: "bodies_error_len", cdecl.} =
  cint(lastError.len)

proc bodiesStagePointer(): ptr uint8 {.exportc: "bodies_stage_ptr", cdecl.} =
  ## The progress note (see stageNote above). Unlike bodies_error_*, this stays
...
proc bodiesStageLength(): cint {.exportc: "bodies_stage_len", cdecl.} =
  cint(stageNoteLen)
...
when isMainModule and defined(emscripten):
  ...
  # freeing the baked board plate, render caches, fonts — everything — while the wasm
  # module stays alive and JS keeps calling bodies_load_replay/bodies_frame. The
